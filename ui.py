...
class MultiPingApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.interval_combo = ttk.Combobox(ctrl, values=interval_labels, textvariable=self.interval_choice, state="readonly", width=10)
        ...
        self.timeout_combo = ttk.Combobox(ctrl, values=timeout_labels, textvariable=self.timeout_choice, state="readonly", width=10)
        ...
        hosts_frame = ttk.LabelFrame(self, text="Hosts"); hosts_frame.pack(side=tk.LEFT, fill=tk.Y, padx=8, pady=6)
        ...
        add_frame = ttk.Frame(hosts_frame); add_frame.pack(side=tk.TOP, fill=tk.X, padx=6, pady=(0,6))
        ...
        right = ttk.Frame(self); right.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=8, pady=6)

        table_frame = ttk.LabelFrame(right, text="Summary (Live)"); table_frame.pack(side=tk.TOP, fill=tk.X)
        ...
        plot_frame = ttk.LabelFrame(right, text="Live Latency (ms)"); plot_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, pady=(6,0))
        self.fig = Figure(figsize=(6,4), dpi=100)
        self.ax = self.fig.add_subplot(111)
        self.ax.set_xlabel("Time (s)"); self.ax.set_ylabel("Latency (ms)"); self.ax.grid(True, alpha=0.3)
    ...
    def _add_host(self):
        ...
        if not h: return
        if h in self.stats:
            messagebox.showinfo("Host exists", f"{h} already in list."); return
        self.host_list.insert(tk.END, h)
    ...
    def _remove_selected(self):
        ...
        for h in sel_hosts:
            ...
            if h in self.stats:
                del self.stats[h]
        for i in reversed(sel): self.host_list.delete(i)
        self._refresh_table(); self._refresh_plot()

    def _bulk_add_dialog(self):
        win = tk.Toplevel(self); win.title("Bulk Add Hosts"); win.geometry("650x420")
        ttk.Label(win, text="Paste hosts, CIDR blocks, or ranges (comma/space/newline separated).").pack(anchor="w", padx=8, pady=(8,4))
        ...
        txt = tk.Text(win, height=12, wrap="word"); txt.pack(fill=tk.BOTH, expand=True, padx=8, pady=6)
        status = tk.StringVar(value=""); ttk.Label(win, textvariable=status).pack(anchor="w", padx=8, pady=(0,6))
        btns = ttk.Frame(win); btns.pack(fill=tk.X, padx=8, pady=(0,8))
        def do_add():
            hosts = parse_hosts(txt.get("1.0", tk.END), limit=10000)
            already = set(self.stats.keys()); added = 0
            for h in hosts:
                if h not in already:
                    self.host_list.insert(tk.END, h)
                    self.stats[h] = HostStats(host=h, count=int(self.count_var.get() or 60))
                    already.add(h); added += 1
            status.set(f"Parsed {len(hosts)} host(s). Added {added}, {len(hosts)-added} skipped (duplicates).")
            self._refresh_table()
    ...
    def _start(self):
        try:
            ...
            if interval_s <= 0: raise ValueError("Interval must be > 0")
            if timeout_ms <= 0: raise ValueError("Timeout must be > 0")
            if count <= 0: raise ValueError("Count must be > 0")
            for h in [self.host_list.get(i) for i in range(self.host_list.size())]:
                if h not in self.stats: self.stats[h] = HostStats(host=h, count=count)
                else: self.stats[h].set_count(count); self.stats[h].reset()
        except Exception as e:
            messagebox.showerror("Invalid settings", str(e)); return

        hosts = [self.host_list.get(i) for i in range(self.host_list.size())]
        if not hosts: messagebox.showinfo("No hosts", "Add at least one host first."); return

        self.summary_shown=False; self.test_active=True
        for h in hosts:
            self.host_manager.start_host(h, interval_s=interval_s, timeout_ms=timeout_ms, max_count=count)
    ...
    def _refresh_table(self):
        ...
        for h, st in sorted(self.stats.items(), key=lambda kv: kv[0].lower()):
            last = st.last(); ip = last.ip if last else ""
            ...
            self.table.insert("", tk.END, values=(h, ip, fmt(last_ms), fmt(mn), fmt(avg), fmt(mx), f"{loss}", f"{recv}", f"{sent}", last_seen))

    def _refresh_plot(self):
        self.ax.cla(); self.ax.set_xlabel("Time (s)"); self.ax.set_ylabel("Latency (ms)"); self.ax.grid(True, alpha=0.3)
        for h, st in sorted(self.stats.items(), key=lambda kv: kv[0].lower()):
            xs, ys = st.series()
            if not xs: continue
            t0 = xs[0]; xs_rel = [x - t0 for x in xs]
            self.ax.plot(xs_rel, ys, label=h, linewidth=1.5)
    ...
    def _show_summary(self):
        ...
        win = tk.Toplevel(self); win.title("Run Summary"); win.geometry("840x440")
        cols=("Host","IP","Sent","Recv","Loss%","Min","Avg","Max")
        tree = ttk.Treeview(win, columns=cols, show="headings")
        for c in cols:
            tree.heading(c, text=c); tree.column(c, width=95 if c not in ("Host","IP") else 140, anchor=tk.CENTER)
        tree.pack(fill=tk.BOTH, expand=True, padx=8, pady=8)

        def fmt(v): return "" if v is None else str(v)
        for (h, ip, sent, recv, loss, mn, avg, mx) in rows:
            tree.insert("", tk.END, values=(h, ip, sent, recv, loss, fmt(mn), fmt(avg), fmt(mx)))

        btns = ttk.Frame(win); btns.pack(fill=tk.X, padx=8, pady=(0,8))
        def copy_to_clipboard():
            header = "\t".join(cols)
            tsv_rows = [header] + ["\t".join(map(lambda x: str(x) if x is not None else '', row)) for row in rows]
            self.clipboard_clear(); self.clipboard_append("\n".join(tsv_rows)); self.update()
    ...
    def _on_close(self):
        try: self.host_manager.stop_all()
